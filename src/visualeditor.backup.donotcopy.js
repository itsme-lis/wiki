(function () {
    ...
    function ensureToolbarState(active) {
        ...
        if (pagetab) pagetab.textContent = active ? "Edit" : "Page";
    }
    ...
    function buildUploadButton() {
        return '<button type="button" class="veuploadbtn" data-widget-action="edit-image"><img src="assets/images/icons/upload.png"></button>';
    }
    function buildWidget(type) {
        if (type === "infobox") {
            return (
                '<div class="vewidget vewidgetinfobox" data-vewidget="infobox">' +
                '<aside class="infobox">' +
                buildWidgetControls(true) +
                '<h3 contenteditable="true">Infobox title</h3>' +
                '<div class="veimagewrap">' +
                '<img class="vewidgetimagepreview" data-image-src="assets/images/examplesquare2.png" src="assets/images/examplesquare2.png">' +
                buildUploadButton() +
                "</div>" +
        ...
        if (type === "media") {
            return (
                '<div class="vewidget vewidgetmedia" data-vewidget="media">' +
                '<figure class="embed embedright">' +
                buildWidgetControls(false) +
                '<div class="veimagewrap">' +
                '<img class="vewidgetmediapreview" data-image-src="assets/images/examplesquare2.png" src="assets/images/examplesquare2.png">' +
                buildUploadButton() +
                "</div>" +
        ...
        if (type === "msg") {
            return (
                '<div class="vewidget vewidgetmsg" data-vewidget="msg">' +
                '<section class="msg">' +
                buildWidgetControls(false) +
        ...
        return (
            '<div class="vewidget vewidgetcard" data-vewidget="' + type + '">' +
    ...
    function setViewMode(next) {
        ...
        root.querySelectorAll(".vemodetab").forEach(function (btn) {
            btn.classList.toggle("isactive", btn.getAttribute("data-mode") === next);
        });
        if (next !== "edit") updateAltViews();
    }
    function hardResetEditor() {
        if (!window.confirm("Are you sure you want to clean the Visual Editor?")) return;
    ...
    function widgetToMd(widget) {
        ...
        if (type === "infobox") {
            ...
            if (image) out.push("image: " + (image.getAttribute("data-image-src") || image.getAttribute("src") || ""));
        ...
        if (type === "media") {
            ...
            return "::media\nurl: " + ((media && (media.getAttribute("data-image-src") || media.getAttribute("src"))) || "") + "\ncaption: " + caption + "\n::";
        }
        if (type === "msg") {
    ...
    function bindControls() {
            ...
            if (actionbtn) {
                ...
                if (widget) {
                    ...
                    if (action === "edit-image") {
                        ...
                        var next = window.prompt("Image URL or path:", (img && (img.getAttribute("data-image-src") || img.getAttribute("src"))) || "");
                        if (img && next && next.trim()) {
                            img.setAttribute("data-image-src", next.trim());
        ...
        root.querySelector(".vehardreset").addEventListener("click", hardResetEditor);
        root.querySelectorAll(".vemodetab").forEach(function (tab) {
            tab.addEventListener("click", function () {
                var mode = tab.getAttribute("data-mode") || "edit";
                if (mode === "reset") hardResetEditor();
...
<div class="vemodetabs">
<button class="vemodetab isactive" data-mode="edit">Edit</button>
<button class="vemodetab" data-mode="preview">Preview</button>
<button class="vemodetab" data-mode="raw">View Raw</button>
<button class="vemodetab vehardreset" data-mode="reset">Reset</button>
</div>
